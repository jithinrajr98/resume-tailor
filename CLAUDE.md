...
2. **Structure** → `lib/groq_client.py:structure_resume()` converts raw text to JSON via Groq/Llama 3.1 70B
...
- `lib/groq_client.py` - Groq API integration (model: llama-3.1-70b-versatile)
- `lib/pdf_generator.py` - `ResumePDF` class handles PDF layout and rendering

### Resume JSON Schema
The structured resume format includes: `name`, `contact`, `summary`, `experience`, `education`, `skills`, `projects`, `certifications`. See `lib/prompts.py` for the full schema definition.
